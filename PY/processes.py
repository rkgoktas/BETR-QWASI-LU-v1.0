...
'''
Modified by Recep Kaya Goktas and Suheyl Cavusoglu, November 2021
when creating BETR-QWASI.
'''
################################################################################
...
class process():
    ...
    def betr_degradation(self):
        ...
        for c in list(self.m.compdict.keys()):
            D[(c,c,procname)]=self.m.chempardict[c]['k_reac']\
                                 ...
                                 *self.m.zdict[c]['bulk']
        '''
        This section is commented out since BETR-QWASI does not have an air
        compartment. RKG, 03.11.2021                                 
    ...
    def betr_advectiveloss(self):
        ...
        D={}
        '''
        The following sections commented out. RKG, 03.11.2021
        ## soil convection
...
#       
        ...
        '''
        # The following loss processes added for BETR-QWASI. RKG, 03.11.2021
        ## sediment burial (modified from above)
        D[(3,3,'burial')]=self.m.par['sedburial']\
                               *self.m.par['A']\
                               *self.m.zdict[3]['solids']
        ## loss by advective outflow in the upper and lower ocean
        D[(1,1,'adv')]= self.m.par['Gup']*self.m.zdict[1]['bulk']
        D[(2,2,'adv')]= self.m.par['Glow']*self.m.zdict[2]['bulk']
        return(D)
    
    def betr_diff_loss(self):
        ''' diffusive loss from the system'''
        # added by RKG and SC for BETR-QWASI. RKG, 11.11.2021
        ## loss by diffusive transfer from upper ocean to air
        # don't change this
        procname=inspect.getframeinfo(inspect.currentframe())[2]       
        D={}
        D[(1,1,procname)] = self.m.par['A']*self.m.par['perc5']*\
                        (1-self.m.par['perc8'])\
                        *((self.m.par['mtc25air']*self.m.zdict[1]['air'])**-1\
                        +(self.m.par['mtc25water']\
                        *self.m.zdict[1]['water'])**-1)**-1
        return(D)
    
    def betr_ocean_partsett(self):
        ''' particle settling from upper ocean to lower ocean '''
        # added by RKG and SC for BETR-QWASI-L. RKG, 13.01.2022
        # describes transfer by settling suspended particles
        # from upper ocean to lower ocean
        # don't change this
        procname=inspect.getframeinfo(inspect.currentframe())[2]
        D={}
        # process description starts here  
        D[(1,2,procname)]=self.m.par['A']*self.m.par['perc5']\
                           *self.m.par['partsett']*self.m.zdict[1]['sussed']
        return(D)
    
    def betr_sediment_ocean_resusp(self):
        ''' sediment resuspension in lower ocean compartments'''
        # don't change this
        procname=inspect.getframeinfo(inspect.currentframe())[2]
        D={}
        # process description starts here
        D[(3,2,procname)]=self.m.par['A']*self.m.par['perc5']\
                           *self.m.par['sedresup']*self.m.zdict[3]['solids']
        return(D)
    
    def betr_ocean_sediment_deposit(self):
        ...
        D[(2,3,procname)]=self.m.par['A']*self.m.par['perc5']\
                           *self.m.par['seddep']*self.m.zdict[2]['sussed']
        return(D)
    
    def betr_ocean_sediment_diff(self):
        ...
        # process description starts here
        D[(2,3,procname)]=self.m.par['A']*self.m.par['perc5']\
                           *self.m.par['diff7water']*self.m.zdict[2]['water']
        D[(3,2,procname)]=D[(2,3,procname)]
        return(D)

""" The following methods are not needed in BETR-QWASI.
    They are commented out. RKG, 11.11.2021
...
################# Fangyuan added processes related with separting particle phases ######################
    def betr_air1_caerosol1_exchange(self):
        ...
        #a=1: Aerosol accommodation coefficient
