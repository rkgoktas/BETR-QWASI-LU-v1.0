...
runID = ['BaP_dyn_SedCap_100E'] # output names 
...
for i in range(0, len(runID)):
    ...
    for y in years[i][1:]:
        ...
        if y == 6:
            print(y)
            #input()
            m.update_solver(solvparamfile = solvfile[i], initfile = 'BaP_endstate5_cap_100E.txt') # update solver with end state of capping scenario
        else:
