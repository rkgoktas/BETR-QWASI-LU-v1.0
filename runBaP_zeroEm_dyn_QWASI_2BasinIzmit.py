...
runID = ['BaP_zeroEm_dyn_QWASI_2BasinIzmit'] # output names 
years = [list(range(1,10))]*len(runID)    # range of modeling run (years)

#emisdir = ['Emission_BDE209_10_comparts']  # emission inventory ('Emissions/annual/')
emisfile = ['Emission_QWASI_zero.txt']*len(runID) # emission inventory ('Emissions/')
...
for i in range(0, len(runID)):
    ...
    #m.update_solver(solvparamfile = solvfile[i], initfile = 'initial_default.txt')
    m.update_solver(solvparamfile = solvfile[i], initfile = 'initial_BaP_from_ss.txt') # BaP init file is used
