...
class Emission:
    ...
    def __init__(self,fn):
        ...
        for lin in lines:
            ...
            #
            #SC changed [m,r,c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,val1,val2,val3,val4,val5,val6,val7,val8,val9,val10] to [m,r,c1,c2,val1,val2] #22.11.2021
            [m,r,c1,c2,c3,val1,val2,val3]=[x.lstrip() for x in \
                                             [x.rstrip() for x in lin.split()]]
            self.em.setdefault(int(float(m)),[]).\
                             append((int(float(r)),int(float(c1)),int(float(c2)),int(float(c3)),float(val1),float(val2),float(val3)))
