...
runID = ['BaP_dyn_SedCap_10E'] # output names 
...
for i in range(0, len(runID)):
    ...
    for y in years[i][1:]:
        ...
        m.update_emissions(emisfile[i]) 
        if y == 6:
            m.update_solver(solvparamfile = solvfile[i], initfile = 'BaP_endstate5_cap_10E.txt') # update solver with end state of capping scenario
        else:
