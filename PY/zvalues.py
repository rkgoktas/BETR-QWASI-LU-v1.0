...
#    Z-values (mol/Pa/m^3) as values.
    ...
    def Z1(self):
        ...
        zdict['water']=0.8*(R*T*self.chempardict[1]['Kaw'])**-1
        # Kqw: suspended sediment - water partitioning coeffcient
        # ATT : replace 0.41 (from Karikoff 1981) by 0.35 (from Seth 1999)
        Kqw=0.41*self.chempardict[1]['Kow'] # organic carbon - water [L/kg]
        Kqw*=self.par['focp1'] # susp. sediment - water [L/kg]
        Kqw*=self.par['rhop45']/1000 #  susp.sediment - water [L/L]
        zdict['sussed']=zdict['water']*Kqw
        zdict['bulk']=(1-self.par['fp1'])*zdict['water']+\
                      self.par['fp1']*zdict['sussed']
        # Add Z-value of air-boundary layer over ocean-water, with T = T_oceanwater # added by HW
        zdict['air']=1/(R*T)
        return(zdict)
    
    def Z2(self):
        '''Z-values for lower ocean'''
        T=self.par[self.compdict[2]['temp_variable']] #temperature field
        zdict={}
        # ATT : factor 0.8 in Zoceanwater=0.8*Zfreshwater from BETR-VBA. Why ?
        zdict['water']=0.8*(R*T*self.chempardict[2]['Kaw'])**-1
        # Kqw: suspended sediment - water partitioning coeffcient
        # ATT : replace 0.41 (from Karikoff 1981) by 0.35 (from Seth 1999)
        Kqw=0.41*self.chempardict[2]['Kow'] # organic carbon - water [L/kg]
        Kqw*=self.par['focp2'] # susp. sediment - water [L/kg]
        Kqw*=self.par['rhop45']/1000 #  susp.sediment - water [L/L]
        zdict['sussed']=zdict['water']*Kqw
        zdict['bulk']=(1-self.par['fp2'])*zdict['water']+\
                      self.par['fp2']*zdict['sussed']
        return(zdict)
    ...
    def Z3(self):
        ...
        Ksedw=0.41*self.chempardict[3]['Kow'] # organic carbon - water [L/kg]
...
###############################################################################
# Unused Z-values commented out. RKG, SC, 15.11.2021
"""    
    def Z1(self):
    ...
    def Z8(self):
        ...
        T=self.par[self.compdict[1]['temp_variable']] #temperature field
        zdict={}
        # aerosol-air partitioning coefficient  Kp=Fom*1500/840*0.6*Koa
