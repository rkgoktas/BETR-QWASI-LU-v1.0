...
runID = ['Kri_dyn_deprem_high_QWASI_2BasinIzmit'] # output names 
years = [list(range(1,2))]*len(runID)    # range of modeling run (years)

#emisdir = ['Emission_BDE209_10_comparts']  # emission inventory ('Emissions/annual/')
emisfile = ['Emission_Kri_const_sed_high.txt']*len(runID) # emission inventory ('Emissions/')
seasparfile = ['seasonal_parameters_QWASI_2BasinIzmit_deprem.txt']*len(runID)#  seasonally varying parameters ('Environment/)
constparfile = ['const_parameters_QWASI_2BasinIzmit.txt']*len(runID)  # seasonally constant parameters ('Environment/')
flowdirectory = ['QWASI_2BasinIzmit_Layered_deprem']*len(runID)  # flows in the atmosphere, ocean and fresh water ('Flows/)

chemdata = ['chemicals_PAH.txt']*len(runID)  # chemical properties ('Chemicals/')
chemnr = [10]*len(runID)  # selection of chemical from chemical properties files
...
for i in range(0, len(runID)):
    ...
    m.update_solver(solvparamfile = solvfile[i], initfile = 'initial_Kri_from_ss_deprem.txt')
