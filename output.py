...
#reload(write_ncfile)
#try:
#from write_ncfile import * ## not used in BETR-QWASI. RKG, 02.12.2021
#except ImportError:
#   from write_ncfile_old import *

from helpers import *
from helpers3 import *
#from secondary_emission_reconstruction import reconstruct_dyn_res  ## not used in BETR-QWASI. RKG, 02.12.2021
...
def write_output_ss(m, fn, units, netcdf, cpk):     # parameter cpk added by HW
    ...
    for c in list(m.compdict.keys()):
        ...
        if 'ng_per_m3' in units:
            out[c]['ng_per_m3']=m.ss_res[idx]*m.chemdict['molmass']*1e9/varr
...
def write_output_ss_QWASI(m, fn, units, cpk):     # parameter cpk added by HW
    ''' Output for steady-state results for BETR-QWASI-L''' # function added by RKG, 19.01.2022
    nlat=int(sqrt(m.matdim/m.nocomp/2))                  # added by HW
    nlon=int(sqrt(m.matdim/m.nocomp/2)*2)                # added by HW
    out={}
    for c in list(m.compdict.keys()):
        ...
        zerozvidx=where(zv_arr==0)
        zv_arr[zerozvidx]=-9999.99
        if c == 1: # upper ocean compartment
        ...
        density_arr = mean(density, axis=1)
        if 'mol' in units:
            out[c]['mol']=m.ss_res[idx]
        if 'kg' in units:
            out[c]['kg']=m.ss_res[idx]*m.chemdict['molmass']/1000.0
        ...
        if 'Pa' in units:
            out[c]['Pa']=m.ss_res[idx]/zv_arr
            out[c]['Pa'][zerozvidx]=0
        if 'mg_per_m3' in units:
            out[c]['mg_per_m3']=m.ss_res[idx]*m.chemdict['molmass']*1e3/varr
            out[c]['mg_per_m3'][zerovolidx]=0
        if 'mg_per_kg' in units:
            out[c]['mg_per_kg'] = m.ss_res[idx]*m.chemdict['molmass']*1e3/(varr*density_arr)
            out[c]['mg_per_kg'][zerovolidx]=0    
        if 'diss_ng_per_L' in units:    # added by RKG, 19.01.2022
            out[c]['diss_ng_per_L'] = out[c]['Pa']\
                                        *mean(m.zdict[c]['water'],axis=1)\
                                        *m.chemdict['molmass']\
                                        *1e9/1e3
        if 'part_ng_per_mg' in units:   # added by RKG, 19.01.2022
            if c == 1:
                out[c]['part_ng_per_mg'] = out[c]['Pa']\
                                            *mean(m.zdict[c]['sussed'],axis=1)\
                                            *m.chemdict['molmass']\
                                            /mean(m.par['rhop45'],axis=1)\
                                            *1e9/1e6
            if c == 2:
                out[c]['part_ng_per_mg'] = out[c]['Pa']\
                                            *mean(m.zdict[c]['sussed'],axis=1)\
...
def write_output_dyn_QWASI(m, fn, units, cpk):     
    ...
    if periods != (timesteps-1)/float(m.nots):
        sys.exit('timesteps of output {0:d} not multiple of '
                 +'seasons ({1:d}): Aborting !'.format(timesteps,m.nots))
    for c in list(m.compdict.keys()):
        #print("in write_output_dyn_QWASI: c = ", c)
        out[c]={}
        idx=arange(c-1,m.matdim,len(list(m.compdict.keys())))
        #print("in write_output_dyn_QWASI: idx = ", idx)
        ## deal with zero volumes / Z-values
        varr=hstack((ones((idx.shape[0],1)),
                     tile(m.vdict[c]['bulk'], (1,periods))))
        #print("in write_output_dyn_QWASI: varr = ", varr)
        zv_arr=hstack((ones((idx.shape[0],1)), 
                       tile(m.zdict[c]['bulk'], (1,periods))))*varr
        zerovolidx=where(varr==0)
        varr[zerovolidx]=-9999.99
        ...
        #print("in write_output_dyn_QWASI: density_arr = ", density_arr)
        if 'mol' in units:
            out[c]['mol']=m.dyn_res[idx]
        if 'kg' in units:
            out[c]['kg']=m.dyn_res[idx]*m.chemdict['molmass']/1000.0
        ...
        if 'Pa' in units:
            ...
            out[c]['Pa'][zerozvidx]=0
        if 'mg_per_m3' in units:
            out[c]['mg_per_m3']=m.dyn_res[idx]*m.chemdict['molmass']*1e3/varr
            out[c]['mg_per_m3'][zerovolidx]=0
        if 'mg_per_kg' in units:
    ...
    # upper ocean compartment concentrations in mg/m3
    for i in range(m.nocells):
        plt.plot(out[1]['mg_per_m3'][i][1:])
    plt.title('Üst Deniz Kompartmanı')
    plt.xlabel('Aylar')
    ...
    for i in range(m.nocells):
        plt.plot(linspace(1,12,12),out[1]['mg_per_m3'][i][timesteps-12:])
    ...
    for i in range(m.nocells):
        plt.plot(out[2]['mg_per_m3'][i][1:])
    plt.title('Alt Deniz Kompartmanı')
    plt.xlabel('Aylar')
    plt.ylabel('Konsantrasyon (mg/m^3)')
    plt.legend(["Merkez Baseni","Doğu Baseni"])
    plt.tight_layout()
    ...
    for i in range(m.nocells):
        plt.plot(out[2]['mg_per_m3'][i][timesteps-12:])
    ...
    plt.plot(out[2]['mg_per_m3'][0][1:])
    plt.title('Merkez Baseni')
    plt.xlabel('Aylar')
    plt.ylabel('Konsantrasyon (mg/m^3)')
    ...
    plt.plot(out[2]['mg_per_m3'][0][timesteps-12:])
    plt.title('Merkez Baseni')
    plt.xlabel('Aylar')
    plt.ylabel('Konsantrasyon (mg/m^3)')
    plt.legend(["Üst Deniz","Alt Deniz"])
    ...
    plt.plot(out[2]['mg_per_m3'][1][1:])
    plt.title('Doğu Baseni')
    plt.xlabel('Aylar')
    plt.ylabel('Konsantrasyon (mg/m^3)')
    plt.legend(["Üst Deniz","Alt Deniz"])
    plt.tight_layout()
    ...
    plt.plot(out[2]['mg_per_m3'][1][timesteps-12:])
    plt.title('Doğu Baseni')
    plt.xlabel('Aylar')
    plt.ylabel('Konsantrasyon (mg/m^3)')
    plt.legend(["Üst Deniz","Alt Deniz"])
    ...
    if m.chemdict['Name'] == "BaP": # Measured values for BaP are used below
        ...
        plt.plot(linspace(1,12,12),out[1]['mg_per_m3'][0][timesteps-12:])
        plt.plot([2,7,8,11],[2.79E-05, 3.26E-04, 4.52E-05, 5.87E-04],'o')
        plt.title('Merkez Baseni - Üst Deniz Kompartmanı')
        plt.xlabel('Aylar')
        plt.ylabel('Konsantrasyon (mg/m^3)')
        ...
        plt.plot([2,7,8,11],[3.11E-05, 1.02E-03, 3.02E-05, 4.26E-04 ],'o')
        plt.title('Merkez Baseni - Alt Deniz Kompartmanı')
        plt.xlabel('Aylar')
        plt.ylabel('Konsantrasyon (mg/m^3)')
        plt.legend(["Model","Ölçüm"])
        plt.tight_layout()
        ...
        plt.plot([2,7,8,11],[3.09E-05, 4.33E-04, 6.12E-04, 1.08E-02],'o')
        plt.title('Doğu Baseni - Üst Deniz Kompartmanı')
        plt.xlabel('Aylar')
        plt.ylabel('Konsantrasyon (mg/m^3)')
        plt.legend(["Model","Ölçüm"])
        plt.tight_layout()
        ...
        plt.plot([2,7,8,11],[4.15E-05, 1.08E-03, 8.74E-02, 5.36E-04],'o')
        plt.title('Doğu Baseni - Alt Deniz Kompartmanı')
        plt.xlabel('Aylar')
        plt.ylabel('Konsantrasyon (mg/m^3)')
        plt.legend(["Model","Ölçüm"])
        plt.tight_layout()
...
def write_bigDlist_txt(bigDlist, fn2, fn3,fn4, fn5,m):             # function added by HW, and modified by FY
    ...
    #output timelist, Volume, Zvalue FY
    timelist = zeros((len(bigDlist2),16))
    Volume = zeros((len(bigDlist2),16))
    Zvalue = zeros((len(bigDlist2),16))
    for i in range(len(bigDlist2)):
        zfromcell=bigDlist2[i][0]-1;zfromcomp=bigDlist2[i][1]
        timelist[i][0:4]=bigDlist2[i][0:4]; Volume[i][0:4]=bigDlist2[i][0:4]; Zvalue[i][0:4]=bigDlist2[i][0:4]
        ...
        #Volume
        Volume[i][4:]=m.vdict[zfromcomp]['bulk'][zfromcell][:]
        #Zvalue
        Zvalue[i][4:]=m.zdict[zfromcomp]['bulk'][zfromcell][:]
