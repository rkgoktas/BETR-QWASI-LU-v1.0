...
"""
monte_carlo = False # SWITCH ON/OFF MONTE CARLO SIMULATIONS, RKG, 20.01.2022
...
runID = ['BaP_TempNorm_ss','BaP_TempPlus1_ss','BaP_TempPlus2'] # output names 
...
seasparfile = ['seasonal_parameters_QWASI_2BasinIzmit_const.txt',\
               'seasonal_parameters_TempPlus1_QWASI_2BasinIzmit_const.txt',\
               'seasonal_parameters_TempPlus2_QWASI_2BasinIzmit_const.txt']  #  seasonally varying parameters ('Environment/)
...
u_envpar = {} # define the uncertain environmental parameters and their k-valuesflowdirectory = ['QWASI_2BasinIzmit_Layered']*len(runID)  # flows in the atmosphere, ocean and fresh water ('Flows/)
...
u_chempar = {} # define the uncertain chemical parameters and their k-values
...
for i in range(0, len(runID)):
    ...
    for iter in range(mc_iter): # Monte Carlo simulations with mc_iter iterations
        ...
        f_ss_bulkConc = open('Output/' + runID[i] + '/ss_bulkConc.txt','w')
        f_ss_bulkConc.write("                     Central Eastern\n")
        f_ss_bulkConc.write("Comp 1 (mg_per_m3): {:.2e} {:.2e}\n".format(output_params[1][0][0],output_params[1][0][1]))
        f_ss_bulkConc.write("Comp 2 (mg_per_m3): {:.2e} {:.2e}\n".format(output_params[2][0][0],output_params[2][0][1]))
        f_ss_bulkConc.write("Comp 3 (mg_per_kg): {:.2e} {:.2e}\n".format(output_params[3][0][0],output_params[3][0][1]))
        f_ss_bulkConc.close()
