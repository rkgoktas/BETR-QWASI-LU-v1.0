...
import processes
import mkflowD
#import mkflowDQ  ## not used in BETR-QWASI. RKG, 02.12.2021
#import mkDmixQ   ## not used in BETR-QWASI. RKG, 02.12.2021
import mkbigD
import emissions
import helpers
import solver
...
#importlib.reload(mkflowDQ)  ## not used in BETR-QWASI. RKG, 02.12.2021
#importlib.reload(mkDmixQ)   ## not used in BETR-QWASI. RKG, 02.12.2021
...
from mkflowD import *
#from mkflowDQ import *  ## not used in BETR-QWASI. RKG, 02.12.2021
#from mkDmixQ import *   ## not used in BETR-QWASI. RKG, 02.12.2021
from mkbigD import *
...
from mc import * # functions for Monte Carlo simulations, RKG, 21.01.2022
...
class Model():
    ...
    def __init__(self,chemical,  run='default',
                 ...
                 monte_carlo = False,    # RKG, 20.01.2022
                 use_correction = False,       #FY
                 track_flows = False,
                 ):    
        ## write model specification to file
        self.monte_carlo = monte_carlo # Monte Carlo simulation, RKG, 20.01.2022
        ...
        ## updates for Monte Carlo simulations, RKG, 23.01.2022
        self.mc_envpar(u_envpar)   #updates environmental parameters for Monte Carlo simulations, RKG, 20.01.2022
        self.mc_chempar(u_chempar) #updates chemical parameters for Monte Carlo simulations, RKG, 23.01.2022
        ...
        #self.update_dvalues_flows_Q()  Commented out. Not used in BETR-QWASI. RKG, SC, 24.11.2021
        #self.update_dvalues_mix_Q()    Commented out. Not used in BETR-QWASI. RKG, SC, 24.11.2021 
    ...
    def update_bigDlist(self):
        ...
        if self.controldict['dumpDmatricesTxt'] in ['1','y','Y','yes','Yes','YES']:   # txt-output added by HW, 15.01.2011
            ...
            fn4=os.path.join('Output', self.run, 'matrixVolume.txt')
            fn5=os.path.join('Output', self.run, 'matrixZvalue.txt')
    ...
    def output_ss_QWASI(self, filename='ss', units=['mol'], cpk=True):  # new function added by RKG, 19.01.2022
        """
        :param string filename: Prefix of the output file(s). The output files
                                will be written to Output/<run>/<filename>_x.
        :param list units: A list of strings to indicate the unit(s) of the
        ...
        ## filename without extension (automatically generated from filetype)
        fn=os.path.join('Output',self.run,filename)
        write_output_ss_QWASI(self,fn,units,cpk)
    ...
    def output_dyn_QWASI(self,filename='dyn_QWASI',units=['mol']):  # new function added by RKG for QWASI, 17.12.2021
        ...
        :param list units: A list of strings to indicate the unit(s) of the
         output. Possible list elements are 'mol', 'mol_per_m3', 'kg',
         'kg_per_m3', 'Pa', 'mg_per_m3', and 'mg_per_kg'.
        :param bool netcdf: Indicates whether netCDF-files are written or not.
        """
        ## filename without extension (automatically generated from filetype)
        fn=os.path.join('Output',self.run,filename)
    ...
    ## Methods to conduct Monte Carlo simulations, RKG, 20.01.2022
    def mc_envpar(self,u_envpar):
        """ updates environemental parameters for Monte Carlo simulations
        """
        if self.monte_carlo == True:
            for param_name in u_envpar.keys():
                N = random.normal() # produce a random number from the normal distribution
                # update the uncertain environmental parameters
                if self.par[param_name].all():
                    self.par[param_name] = exp(N*sigma(self.par[param_name],u_envpar[param_name])\
                                           + mu(self.par[param_name],u_envpar[param_name]))
                else:
                    for i in range(len(self.par[param_name])):
                        for j in range(self.nots):
                            if self.par[param_name][i][j] != 0.0:
                                self.par[param_name][i][j] = exp(N*sigma(self.par[param_name][i][j],u_envpar[param_name])\
                                           + mu(self.par[param_name][i][j],u_envpar[param_name]))
                            else:
                                self.par[param_name][i][j] = 0.0
    def mc_chempar(self,u_chempar):
        """ updates chemical parameters for Monte Carlo simulations
        """
        if self.monte_carlo == True:
            for param_name in u_chempar.keys():
                N = random.normal() # produce a random number from the normal distribution
                # update the uncertain chemical parameters
                if self.chemdict[param_name] > 0.0:
                    self.chemdict[param_name] = exp(N*sigma(self.chemdict[param_name],u_chempar[param_name])\
                                           + mu(self.chemdict[param_name],u_chempar[param_name]))
                elif self.chemdict[param_name] < 0.0:
                    self.chemdict[param_name] = log10(exp(N*sigma(10**self.chemdict[param_name],u_chempar[param_name])\
                                                + mu(10**self.chemdict[param_name],u_chempar[param_name])))
                else:
                    self.chemdict[param_name] = 0.0
